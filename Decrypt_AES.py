...
def dec_decrypt_byte_as_nibbles( nibble_quadruple, key_quadruple ):
    ...
    # use all the functions in order as described in the instructions
    return (dec_xor(dec_s_box(dec_swap_last_two(dec_subtract_last_two_first_two(dec_xor(dec_s_box(dec_swap_last_two(dec_xor(nibble_quadruple, key_quadruple))), key_quadruple)))), key_quadruple))
...
def dec_xor( nibbles, key ):
    ans = bytearray( len(nibbles) )
    # calculate xor for each nibble using ' ^ '
    for index in range(len(nibbles)):
        ans[index] = nibbles[index] ^ key[index]
...
def dec_swap_last_two( nibbles ):
    ...
    # swap then last and the second last indexes
...
def dec_s_box( nibbles ):
    ans = bytearray(len(nibbles))
    # go through each nibble and replace using their hexadecimals
    for index in range(len(nibbles)):
        if(nibbles[index] == 0x3):
            ans[index] = 0x1
        elif(nibbles[index] == 0x2):
            ans[index] = 0x3
        elif(nibbles[index] == 0x1):
            ans[index] = 0x0
        elif(nibbles[index] == 0x0):
            ans[index] = 0x2
...
def dec_subtract_last_two_first_two( nibbles ):
    ans = bytearray( nibbles )
    # substract and make sure always positive
    ans[0] = (ans[0]-ans[2])%4
    ans[1] = (ans[1]-ans[3])%4
